defmodule Rumbl.VideoControllerTest do
  ...
  setup %{conn: conn} = config do
    if username = config[:login_as] do
      ...
      conn = assgin(conn(), :current_user, user)
  ...
  test "lists all user's videos on index", %{conn: conn, user: user} do
    ...
    assert html_response(conn, 200) =~ ~r/List videos/
    assert String.contain?(conn.resp_body, user_video.title)
    refute String.contain?(conn.resp_body, other_video.title)
  end
  ...
  alias Rumbl.Video
  @vaild_attrs %{url: "http://youtu.be", title: "vid", description: "a vid"}
